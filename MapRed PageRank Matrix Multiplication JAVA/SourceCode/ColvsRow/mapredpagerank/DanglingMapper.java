...
// We do DR where D is 1XV and R is VX1 so we will get just one value for dangling node.
public class DanglingMapper extends Mapper<Object, Text, LongWritable, PageNode> {
    ...
    public void map(Object _k, Text lines, Context ctx) throws IOException, InterruptedException {
         ...
         if("D".equals(id.trim())){
            ...
            ctx.write(new LongWritable(row),new PageNode(val,col,id)); //for the second matrix emit the row number;
         }
