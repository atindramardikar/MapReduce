...
// Version1: Column by Row implementation of PageRank.
...
public class MapRedPageRank {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 1; i <= 10; i++) {
            long dpr = dangling(new Path(args[1] + "/matrices"), new Path(args[1] + "/iter" + (i - 1)), new Path(args[1] + "/intermediatedpr" + i), new Path(args[1] + "/dR" + i));
    ...
    // Two jobs:
    // First job to calculate contributions.
    // Second job to add contributions and calculate final Pr for everypage.
    private static long dangling(Path input, Path input1, Path output, Path finaloutput) throws Exception {
        Configuration conf = new Configuration();
        ...
        Job job2 = Job.getInstance(conf, "add");
        job2.setJarByClass(MapRedPageRank.class);
        job2.setMapperClass(AddMapper.class);
        job2.setReducerClass(DanglingAddReducer.class);
        ...
        job2.waitForCompletion(true);
        long newDanglingPR = job2.getCounters().findCounter(CountersPR.danglingNodePR).getValue();
    ...
    private static void multi(long totalNodes, Path input, Path input1, Path output, Path finaloutput, long dpr) throws IOException, InterruptedException, ClassNotFoundException, Exception {
        ...
        Job job2 = Job.getInstance(conf, "add");
        job2.getConfiguration().setLong("V", totalNodes);
        job2.getConfiguration().setLong("DanglingNodePR", dpr);
        job2.setJarByClass(MapRedPageRank.class);
        job2.setMapperClass(AddMapper.class);
        job2.setReducerClass(AddReducer.class);
        job2.setMapOutputKeyClass(LongWritable.class);
        job2.setMapOutputValueClass(DoubleWritable.class);
        job2.setOutputKeyClass(Text.class);
